...
makeKeyboardColumn = function () {
  $('#letterKeyAnchor').empty(); // clear tray first. shouldn't be needed but safety checks are good
  for (let i = 65; i < 91; i++) { // unicode #s for uppercase letters
    let letterVar = String.fromCharCode(i);
/*    let letterButtonDiv = $('<button>');
    letterButtonDiv.attr('id', letterVar)
      .addClass("letterKey")
      .txt(letterVar);*/
    $('#letterKeyAnchor').append(`<button id="${letterVar}" class="letterKey">${letterVar}</button>`);
  }
}
...
checkPlayerGuess = function (playerGuess) {
  ...
  if (lettersToWin == 0) {
    $('#letterKeyAnchor').empty(); // keyboard empties out
...
// react to onscreen keyboard clicks
